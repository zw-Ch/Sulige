...
def get_xy(input_data, x_length=32, y_length=4, style="list", label_dim=None):
    seq_list, data_arr, label_arr = [], None, None
    data_length = len(input_data)
    x_y_length = x_length + y_length
    if style == "list":     # list类型，用于RNN网络
        for i in range(data_length - x_y_length + 1):
            if input_data.ndim == 2:
                data = input_data[i: (i + x_length), :]
                if label_dim is not None:
                    label = input_data[(i + x_length): (i + x_length + y_length), label_dim].reshape(-1, 1)
                else:
        ...
        for i in range(data_length - x_y_length + 1):
            if input_data.ndim == 2:
                ...
                raise TypeError('!')
            data, label = data.reshape(1, -1), label.reshape(1, -1)
            if i == 0:
                data_arr, label_arr = data, label
            else:
                data_arr, label_arr = np.vstack([data_arr, data]), np.vstack([label_arr, label])
...
# 一个井的信息
class Well(object):
    def __init__(self, name, xy, data):
        self.name = name            # 井名
        self.xy = xy                # 坐标
        self.data = data            # 日产气量

    def plot(self):
        plot_array(a=self.data, title=self.name, y_name="日产气量")


# 多口油井的信息
class WellInfo(object):
    def __init__(self, name, root="../dataset/static_data/new_SD", scale=None, gtype="knn", k=2, s=20, data=None, time=None,
    ...
    def read_data(self):
        if self.name == "su47":
            ...
            file = ["47(1)", "47(2)", "47(3)", "47(4)", "47(5)", "47(6)", "47(7)", "47(8)", "47(9)", "47(10)", "47(11)",
                    "120(1)", "120(2)", "苏14(1-100)", "苏14(101-200)", "苏14(201-300)", "苏14(301-400)", "苏14(401-500)",
                    "苏14(501-600)", "苏14(601-700)", "苏14(701-800)", "苏14(801-900)", "苏14(901-1000)",
                    "苏14(1101-1200)", "苏14(1201-1300)", "苏14(1301-1400)", "48(1)", "48(2)", "48(3)", "48(4)", "48(5)",
