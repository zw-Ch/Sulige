...
nat_len = 5                    # 满足自然连续条件的长度，用于定位：自然连续生产段结束时刻，default=100
mea_len = 5                      # 满足措施连续条件的长度，用于定位：措施连续生产段结束时刻，default=70
aba_len = 4                     # 满足经济废弃条件的长度，用于定位：间歇生产段结束时刻/经济废弃段开始时刻，default=150
...
读取数据
"""
df_ = get_sheet(osp.join(data_ad, 'product_data'), name)             # 动态数据，包括日期、产量、套压、油压
df = df_[df_['日产气量(10⁴m³/d)'] != 0]
...
cp_diff = np.diff(cp)
dgo_diff = np.diff(dgo)
cp_diff = np.insert(cp_diff, -1, 0)
if (dyc_class == 'Ⅰ类井') | (dyc_class == 'Ⅱ类井'):
    idx_mea = np.argwhere((dgo > q_sc * 0.8) & (cp_diff < 0.001) & (dgo < 0.3)).reshape(-1)
elif dyc_class == 'Ⅲ类井':
    idx_mea = np.argwhere((dgo > q_sc * 0.5) & (cp_diff < 0.002) & (dgo < 0.3)).reshape(-1)
...
if (dyc_class == 'Ⅰ类井'):
    # idx_aba = np.argwhere((eur_new < rrg) & (dgo < 0.1) & (cp > 2)).reshape(-1)
    idx_aba = np.argwhere((dgo < 0.1) & (cp > 2)).reshape(-1)
elif (dyc_class == 'Ⅱ类井'):
    idx_aba = np.argwhere((dgo < 0.1) & (cp > 3)).reshape(-1)
elif (dyc_class == 'Ⅲ类井'):
    idx_aba = np.argwhere((dgo < 0.1) & (cp > 7)).reshape(-1)
...
if idx_aba.shape[0] == 0:
    ...
    for i_aba in range(idx_aba.shape[0] - 1):           # 修改处
        ...
        if a.shape[0] < aba_len:
            ...
            if idx_aba[i_aba] < idx_mea[i_mea]:
                continue
            else:
                break
...
cp_whole = df.loc[:, '套压(MPa)'].values.reshape(-1)
dgo_whole = df.loc[:, '日产气量(10⁴m³/d)'].values.reshape(-1)

start_date = tran_locate(start_date, range_len, dgo_whole)
criti_date = tran_locate(criti_date, range_len, dgo_whole)
measu_date = tran_locate(measu_date, range_len, dgo_whole)
aband_date = tran_locate(aband_date, range_len, dgo_whole)
...
plt.sca(ax1)
plt.plot(dgo_whole, "g--", label="Gas production", alpha=0.5)
y_min, y_max = 0, np.max(t) * 0.5
...
plt.sca(ax1.twinx())
# cp = cp[idx_nan]
plt.plot(cp_whole, "y--", label="Casing pressure")
